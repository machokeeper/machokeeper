...
command -v codesign >/dev/null || { echo "no codesign; not macOS"; exit 0; }
command -v nix >/dev/null || { echo "nix not on PATH"; exit 1; }

# Ask the harness's own picker for the smallest repairable path+file.
read -r STORE_PATH REL < <(python3 - <<'PY'
...
if codesign --verify "$FILE" 2>/dev/null; then
  echo "FAIL: codesign accepted a supposedly-broken binary"; exit 1
fi
